...
| Operational test media | The Worker smoke test imports `pig_icon.png` from the public [`postmanlabs/httpbin` image endpoint](https://github.com/postmanlabs/httpbin/blob/master/LICENSE). The upstream repository distributes that fixture under ISC; the test now records the upstream contributors, license, file, and repository in the governed asset. | Pass; no unattributed test-media copy remains. |
